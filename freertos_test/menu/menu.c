...
void Menu_Input_Field(uint8_t current_key,uint8_t attributes,struct input_buffer *inp_buf)
{
	...
	if((Menu_Input_Buf_To_Int(inp_buf)>INPUT_MAX_VALUE)||(Menu_Input_Buf_To_Int(inp_buf)<(-INPUT_MAX_VALUE)))//выход за диапазон-удал€ем последний введенный символ
	{
		Menu_Input_Field_Shift(DIRECTION_RIGHT,inp_buf);
	}
